import cv2
import numpy as np
import os

recognizer = cv2.face.LBPHFaceRecognizer_create()
recognizer.read('trainer/trainer.yml')
cascadePath = "haarcascade_frontalface_default.xml"
faceCascade = cv2.CascadeClassifier(cascadePath);

font = cv2.FONT_HERSHEY_SIMPLEX

#iniciate id counter
id = 0

# names related to ids: e.g. David Hoyos is id 1 when data_gathering and displayed with diff ID
names = ['none', 'User: DH210389', 'Alejandro', 'Adriana', 'Paola', 'DOwen','Jorge','Next']

# Initialize and start realtime video capture
cam = cv2.VideoCapture(0)
cam.set(3, 640) # set video widht
cam.set(4, 480) # set video height

# Define min window size to be recognized as a face
minW = 0.15*cam.get(3) #0.1 by default on both
minH = 0.15*cam.get(4) #higher factor means person needs to get closer to camera for detection

while True:
    ret, img =cam.read()
    gray = cv2.cvtColor(img,cv2.COLOR_BGR2GRAY)

    faces = faceCascade.detectMultiScale(
        gray, # Greyscale image input
        scaleFactor = 1.2, # Param specifying how much the img size is reduced at each image scale
        minNeighbors = 5, # Param, with a higher value gives lower false positives
        minSize = (int(minW), int(minH)), # min rectangle to be considered a face
       )

    for(x,y,w,h) in faces:
        cv2.rectangle(img, (x,y), (x+w,y+h), (200, 200, 75), 2)
        id, confidence = recognizer.predict(gray[y:y+h,x:x+w])

        # Check if confidence is less than 100 | "0" is perfect match
        if (confidence < 100):
            id = names[id] # displays names
            #-------------!!!!!!!!!!!!!!-----------------
            #id = id # displays IDs this part will send the ID with a service to the welcome script
            confidence = "  {0}%".format(round(100 - confidence))
        else:
            id = "unknown"
            confidence = "  {0}%".format(round(100 - confidence))

        cv2.putText(img, str(id), (x+5,y-5), font, 1, (200,200,200), 2)
        cv2.putText(img, str(confidence), (x+50,y+h-5), font, 0.75, (255,255,0), 1)

    cv2.imshow('camera',img)

    k = cv2.waitKey(10) & 0xff # Press 'ESC' for exiting video
    if k == 27:
        break

# Do a bit of cleanup
print("\n [INFO] Exiting Program and cleanup stuff")
cam.release()
cv2.destroyAllWindows()
